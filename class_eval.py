...
def split_training_testing(data, p_test):
    '''Assumes data is a list of length of at least 10
    and p_test is a positive number less than 100.
    Takes a list of data points and splits it randomly into a training set
    and a testing set, where testing set is p_test percent of the data.
    Returns the training and testing sets, each as a list of data points.
    '''
...
def confusion_matrix(vals_predicted, vals_actual, pos_val):
    '''Takes a non-empty list of predicted labels, a non-empty list of
    actual labels, and the value for the positive label and returns the number
...
def accuracy(tp, fp, tn, fn):
    '''Assumes tp, fp, tn, and fn are non-negative integers.
    Returns the accuracy, which is (tp + tn) / (tp + fp + tn + fn).'''
...
def sensitivity(tp, fn):
    '''Assumes tp and fn are non-negative integers.
    Returns the sensitivity, which is tp / (tp + fn).'''
...
def specificity(tn, fp):
    '''Assumes tn and fp are non-negative integers.
    Returns the specificity, which is tn / (tn + fp).'''
...
def pos_pred_val(tp, fp):
    '''Assumes tp and fp are non-negative integers.
    Returns the positive predictive value, which is tp / (tp + fp).'''
...
def neg_pred_val(tn, fn):
    '''Assumes tn and fn are non-negative integers.
    Returns the negative predictive value, which is tn / (tn + fn).'''
...
def print_eval_metrics(vals_predicted, vals_actual, pos_val):
    '''Takes a list of predicted labels, a list of actual labels, and the value
    for the positive label and prints the accuracy, sensitivty, specificity,
